...
def _dummy_pool_for_get_order():
    class DummyConn:
        async def fetchrow(self, sql: str, *args: Any, **kwargs: Any) -> dict[str, Any] | None:
            ...
            if "SELECT id, user_id" in sql:
                return {"id": TEST_ORDER_ID, "user_id": "u2", "item_name": "x", "quantity": 1}
            return None
    ...
    class DummyAcquireCM:
        ...
        async def __aexit__(self, exc_type: type | None, exc: BaseException | None, tb: object | None) -> bool:
            return False
...
def _dummy_pool_for_list_user():
    class DummyConn:
        async def fetchall(self, sql: str, *args: Any, **kwargs: Any) -> list[dict[str, Any]]:
...
def test_list_user_orders_forbidden(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        r = client.get(f"/orders/user/u2")
        if r.status_code != 403:
            raise AssertionError(f"Expected status 403 for forbidden, got {r.status_code}")
    finally:
...
def test_get_order_forbidden(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        if r.status_code != 403:
            raise AssertionError(f"Expected status 403 for forbidden order fetch, got {r.status_code}")
    finally:
        app.dependency_overrides = orig
